...
PREDICTOR::PREDICTOR(void){
    ...
    _perceptron = new PerceptronPredictor();
...
bool PREDICTOR::GetPrediction(UINT32 PC){
    prediction_perceptron = _perceptron->GetPrediction(PC);
    prediction_tage = _tage->GetPrediction(PC);
    
    int index = (PC >> 2 ^ GHR) & PCmask_hybrid;
    return HybridTable[index] > 1 ? prediction_tage : prediction_perceptron;
}
...
void PREDICTOR::UpdatePredictor(UINT32 PC, bool resolveDir, bool predDir, UINT32 branchTarget){
    ...
    _perceptron->UpdatePredictor(PC, resolveDir, prediction_perceptron, branchTarget);

    index = (PC >> 2 ^ GHR) & PCmask_hybrid;
    
    if (prediction_tage == resolveDir && prediction_perceptron != resolveDir)
    {
...
PerceptronPredictor::PerceptronPredictor(void){
...
bool PerceptronPredictor::GetPrediction(UINT32 PC){
    int key = get_key(PC);
    int y = _bias[key];
...
void  PerceptronPredictor::UpdatePredictor(UINT32 PC, bool resolveDir, bool predDir, UINT32 branchTarget){
